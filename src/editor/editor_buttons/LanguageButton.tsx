    ...
    return (
        <>
            <Button class="flex grow basis-1 flex-col items-center md:grow-0 md:flex-row" onClick={() => setActive(true)}>
                ...
                <span class="hidden whitespace-nowrap sm:inline">{props.currentLangName.toUpperCase()}</span>
            </Button>
            <Show when={active()}>
                <ModalContainer class="flex h-1/2 flex-col" onClose={() => setActive(false)}>
                    <span class="mb-2 block w-full text-center text-2xl text-primary-400">Languages</span>
                    <span class="mb-2 text-primary-400">These only apply to the shader settings, the website is currently only available in english.</span>
